...
gltfLoader.load(
    ...
    (gltf) => {
        ...
        scene.add(avatar)
        // Supposons que vous ayez déjà créé votre caméra et votre objet "avatar"
        // avatar est votre objet 3D que vous souhaitez orienter vers la position donnée

        // Position de l'élément à cibler
        const targetPosition = new THREE.Vector3(10, -11, 0);

        // Vecteur direction entre la caméra et l'élément ciblé
        const direction = new THREE.Vector3().subVectors(targetPosition, avatar.position).normalize();

        // Vecteur "up" (haut)
        const upVector = new THREE.Vector3(8, 3, 0);

        // Création de la matrice LookAt
        const lookAtMatrix = new THREE.Matrix4().lookAt(avatar.position, targetPosition, upVector);

        // Application de la matrice LookAt à la rotation de l'avatar
        avatar.quaternion.setFromRotationMatrix(lookAtMatrix);
...
gltfLoader.load
    (
        ...
        (gltf) => {
            ...
            bubbleSpeech.scale.set(0.7, 0.7, 0.7)
            bubbleSpeech.position.x = -0.5
            bubbleSpeech.position.y = -7.5
...
// how to make camera look to the models 
...
// Add the camera
const camera = new THREE.PerspectiveCamera(50, sizes.width / sizes.height, 0.1, 600)
...
fontLoader.load('./assets/fonts/pixel.json', (font) => {
    // Crée la géométrie du texte
    ...
    // Crée le matériau pour le texte
    const material = new THREE.MeshBasicMaterial({ color: 0x00ff00 });

    // Crée l'objet de maillage pour le texte
    const mesh = new THREE.Mesh(geometry, material)
    mesh.position.x = 10
    mesh.position.y = 1000
    mesh.position.z = -15

    // Ajoute le texte à la scène
    scene.add(mesh)
});
